...
do
  ...
  l=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $3}'`
  m=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $4}'`
  n=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $5}'`
  ...
  do
    ...
    NAME="${TAG}_${NAMEONE}${j}_${NAMETWO}${k}_${l}_${MODE}_${m}"
